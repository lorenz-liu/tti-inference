...
# --- Configuration ---
DEFAULT_MODEL_PATH = './model_folder/ViT/best_model.pt'
DEFAULT_DATA_PATH = './processed_dataset_balanced/test/'
BATCH_SIZE = 32
...
def load_model(model_path, device):
    ...
    try:
        ...
        model = ROIClassifierViT(num_hoi_classes=2)
            ...
            if model_info.get('no_depth', False):
                model = ROIClassifierEfficientNetNoDepth(num_classes=2, efficientnet_version=version)
            else:
                model = ROIClassifierEfficientNet(num_classes=2, efficientnet_version=version)
        else:
        ...
        for frames, labels in tqdm(loader, desc="Evaluating model"):
            ...
            if model_info['model_type'] == 'ViT':
                ...
                if len(predicted.shape) > 1:
                    predicted = predicted[:, 1] if predicted.shape[1] > 1 else predicted[:, 0]
...
def calculate_metrics(preds, labels, probs):
    ...
    report = classification_report(labels, preds, target_names=['No-TTI', 'TTI'], output_dict=True)
...
def save_results(metrics, model_info, model_path, save_dir):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("           Predicted\n")
        f.write("         No-TTI  TTI\n")
        f.write(f"Actual No-TTI  {metrics['confusion_matrix'][0][0]:4d}  {metrics['confusion_matrix'][0][1]:4d}\n")
        f.write(f"       TTI     {metrics['confusion_matrix'][1][0]:4d}  {metrics['confusion_matrix'][1][1]:4d}\n")
...
def plot_results(metrics, model_info, save_dir):
    ...
    metrics_values = [metrics['accuracy'], metrics['sensitivity'], 
                     metrics['specificity'], metrics['precision'], metrics['f1_score']]
    ...
    plt.title(f'Performance Metrics - {model_info["model_name"]}', 
              fontsize=14, fontweight='bold')
...
def visualize_predictions(frames, labels, preds, model_info, save_dir, num_samples=12):
    """Displays a grid of images with their true and predicted labels."""
    indices = np.random.choice(len(frames), size=min(num_samples, len(frames)), replace=False)
    ...
    fig.suptitle(f'Sample Predictions - {model_info["model_name"]}', fontsize=16, fontweight='bold')
...
def display_results(metrics, model_info):
    ...
    print("           Predicted")
    print("         No-TTI  TTI")
    print(f"Actual No-TTI  {metrics['confusion_matrix'][0][0]:4d}  {metrics['confusion_matrix'][0][1]:4d}")
    print(f"       TTI     {metrics['confusion_matrix'][1][0]:4d}  {metrics['confusion_matrix'][1][1]:4d}")
    print("="*60)

def main():
    parser = argparse.ArgumentParser(description="Evaluate a trained TTI-classifier model.")
    ...
    # 4. Evaluate
    predictions, true_labels, probabilities = evaluate(model, loader, device, model_info)
    ...
    save_dir = args.save_dir if args.save_dir else get_model_save_directory(args.model_path)
    
    # 8. Save results
    results_path, summary_path = save_results(metrics, model_info, args.model_path, save_dir)
    ...
    predictions_path = visualize_predictions(frames, true_labels, predictions, model_info, save_dir)
