...
def visualize_frame_analysis(json_path, gng_video_path, frame_to_debug):
    ...
    print(f"GO color (BGR): {GO_COLOR}, Pixels found: {go_pixels}")
    print(f"NOGO color (BGR): {NOGO_COLOR}, Pixels found: {nogo_pixels}")
    ...
    vis_main_small = cv2.resize(vis_main, (vis_w, vis_h), interpolation=cv2.INTER_NEAREST)
    cv2.putText(vis_main_small, "Full Frame + BBox", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    ...
    vis_go_mask = cv2.resize(vis_go_mask, (vis_w, vis_h), interpolation=cv2.INTER_NEAREST)
    cv2.putText(vis_go_mask, "GO Mask", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

    vis_nogo_mask = cv2.cvtColor(nogo_mask, cv2.COLOR_GRAY2BGR)
    vis_nogo_mask = cv2.resize(vis_nogo_mask, (vis_w, vis_h), interpolation=cv2.INTER_NEAREST)
    cv2.putText(vis_nogo_mask, "NOGO Mask", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Debug GNG analysis for a single frame.")
    parser.add_argument('--filtered_json', required=True, help='Path to the filtered JSON file.')
    parser.add_argument('--gng_video', required=True, help='Path to the GO/NOGO video file.')
    parser.add_argument('--frame_number', required=True, type=int, help='The specific frame number to debug.')
